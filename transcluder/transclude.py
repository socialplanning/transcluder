...
def transclude(document, document_url, 
    ...
    for target in target_links: 
        source_url = get_include_url(target, document_url, variables) 

        if source_url is None: 
            continue

        try: 
            subdoc = fetch(source_url)

            if should_recurse(source_url): 
                transclude(subdoc, source_url, variables, fetch, 
...
def merge(target, subdoc, source_url): 
    """
...
def get_include_url(target, document_url, variables): 
    """
    get and normalize the href attribute of the link 'target'.  
    1. expand uri template variables with the dictionary 'variables' 
    ...
    variables: a dictionary used for uri template expansion 
    """
    source_url = target.get("href", None)

    if source_url is None or len(source_url) == 0: 
        attach_warning(target, "no href specified")
        return None

    source_url = uritemplates.expand_uri_template(source_url, variables)
    return urlparse.urljoin(document_url, source_url)


def attach_warning(target, message): 
    """
